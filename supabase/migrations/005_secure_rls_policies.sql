...
-- Anon: can read public orders (delivered, cancelled) for tracking
...
-- Authenticated: can read/write own order items
DROP POLICY IF EXISTS order_items_own ON order_items;
CREATE POLICY order_items_own ON order_items
  FOR ALL TO authenticated
  USING (order_id IN (SELECT id FROM orders WHERE customer_phone = auth.email()))
  WITH CHECK (order_id IN (SELECT id FROM orders WHERE customer_phone = auth.email()));
...
-- Anon: can read public pre-orders (for menu planning)
...
-- Authenticated: can read/write own pre-orders
...
-- Admin: can manage all pre-orders
...
-- Anon: no access to payment details
...
CREATE POLICY payment_intents_own ON payment_intents
  FOR SELECT TO authenticated
  USING (order_number IN (SELECT order_number FROM orders WHERE customer_phone = auth.email()));

-- Authenticated: can create own payment intents
...
CREATE POLICY payment_intents_own_create ON payment_intents
  FOR INSERT TO authenticated
  WITH CHECK (order_number IN (SELECT order_number FROM orders WHERE customer_phone = auth.email()));

-- Admin: can manage all payment intents
...
-- Public: can read all profiles (for reviews, public info)
DROP POLICY IF EXISTS profiles_public_read ON profiles;
CREATE POLICY profiles_public_read ON profiles
  FOR SELECT TO anon, authenticated
  USING (true);

-- Own: can update own profile
...
-- ============================================
-- 6. products, product_categories, inventory — Public Read, Admin Write
-- ============================================
ALTER TABLE products ENABLE ROW LEVEL SECURITY;
ALTER TABLE product_categories ENABLE ROW LEVEL SECURITY;
ALTER TABLE inventory ENABLE ROW LEVEL SECURITY;

-- Public: can read all products/categories/inventory
...
-- ============================================
-- 7. customers — Admin only (no public access)
-- ============================================
...
-- ============================================
-- Revoke public access, grant to authenticated only where needed
