-- ============================================
-- Bite Me Baby — Migration 009: Fix 007 runtime bug — `extract_epoch` missing on live DB
-- Date: 2026-09-19 (Stripe LIVE VERIFY session)
--
-- PROBLEM (live evidence 2026-09-19): create_order_with_items() on the live
-- project (ivkdfognyiwjcmrhcnwz) fails every call with:
--   42883 function extract_epoch(timestamp with time zone) does not exist
-- Root cause: migration 007 used non-portable extract_epoch(); the portable
-- form is extract(epoch from ...) (already used by migration 008).
--
-- FIX: CREATE OR REPLACE same function with portable expression. Idempotent.
-- AFTER: run 008_payment_state_machine_and_phase_d.sql (payment/order RPCs are
-- not yet live — verified 2026-09-19 via service_role probes -> PGRST202).
-- ============================================

BEGIN;

-- ============================================
-- 1. RPC: create_order_with_items (fixed body — identical to 007 except the
--    order_id epoch expression). Decoded from migration 007 verbatim.
-- ============================================
...
-- ===== 6. DELIVERY FEE (authoritative server rules; mirrors
  ...
  -- FIX (009): extract_epoch(timestamptz) does not exist on the live project.
  --            Portable equivalent: extract(epoch from clock_timestamp()).
...
-- ============================================
-- 2. EXECUTE permission — authenticated only (re-assert, idempotent)
-- ============================================
