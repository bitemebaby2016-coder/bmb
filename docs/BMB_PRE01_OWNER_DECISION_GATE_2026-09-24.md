...
|---|---|---|
| A. 3 วันข้างหน้า (แนะนำ) | สั่งล่วงหน้าได้ D+1 ถึง D+3, ต่อ round ตาม cutoff ของ scheduled_date | เพิ่ม validation ใน `create_order_with_items` (PRE_ORDER): `scheduled_date BETWEEN today+1 AND today+3` + ERR code ใหม่ |
...
- [ ] 1B. เพิ่ม cutoff แยกสำหรับ PRE_ORDER (เช่น ปิดก่อน same-day cutoff) — ต้องเพิ่ม column/setting ใหม่

**max_preorder_days ค่าที่ตกลง:** ________
...
|---|---|---|
| A. ยกเลิกได้ก่อน cutoff เท่านั้น (แนะนำ) | หลัง cutoff ของ scheduled_date+round → ปฏิเสธ `ERR_CANCEL_AFTER_CUTOFF` | แก้ `cancel_order`: เทียบ NOW() กับ cutoff ของ round ที่ผูก order; capacity+inventory ถูกคืนเฉพาะเมื่อยกเลิกสำเร็จ |
...
ลงชื่อตอบ: 1.___ 2.___ 3.___ (อ้างตัวเลือก A/B/C)
