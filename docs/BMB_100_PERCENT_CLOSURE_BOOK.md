...
| SEC-02 | AI key | ไม่มี secret ใน client bundle (AI proxy ผ่าน EF) | **CODE-DONE (await deploy EF)** — `supabase/functions/ai-proxy` (verify_jwt=true, key อยู่ env, guardrail segment ฝั่ง server) + `aiService`/`aiToolCalling`/recommend route ผ่าน proxy — client ไม่มี key อีกต่อไป | `ai-proxy/index.ts` + config.toml + `aiService.ts` | EF deploy | P0 | 4 | build + grep dist ไม่มี key; ทดสอบ chat ผ่าน EF | key หาไม่เจอใน bundle; chat ใช้งานได้เหมือนเดิม | **Deploy (owner): `supabase functions deploy ai-proxy` + secrets set** → S-1 ปิด |
...
| KIT-01 | Production queue | batch/production queue ต่อ round | **CODE-DONE (server)** — migration 019 `production_batches`+`production_batch_items`, RPC `create_production_batch`/`kitchen_queue`, client `kitchenService.ts` (queue + batch summary) · **UI panel (admin) เป็นงานค้างถัดไปในเฟสนี้** | migration 019 + tests 115/115 | ORD-01 | P1 | 2 | deploy 019 → SQL suite + e2e: ออเดอร์→batch→ready | ครัวเห็นงานต่อ batch จริง | **Deploy (owner): `supabase db push`** แล้วรัน `e2e/contracts_019_kitchen.sql` |
| KIT-02 | Recipes/BOM | recipe → ingredient requirement | **CODE-DONE** — ตาราง `recipes` (BOM) + RPC `get_inventory_requirements` + pure `bomFeasibility()` (availability คิดจากสูตร) + seed prod-1..4 | migration 019 + `src/lib/kitchenService.ts` + tests | INV-01 | P1 | 2 | deploy 019 → SQL suite; สร้างสูตร→สั่ง→สต็อกลด | availability คิดจากสูตรได้ | **Deploy (owner): `supabase db push`** |
| INV-01 | Stock deduct | หักสต็อกเมื่อ order ยืนยัน (server) | **CODE-DONE (await deploy 019)** — `deduct_inventory_for_order`/`restore_inventory_for_order` (idempotent, admin) + hook ใน `transition_order_status` (confirm→deduct, cancel/fail→restore) + audit ผ่าน inventory_transactions | migration 019 + migrate 018 audit + tests | SEC-03 | P0 | 2 | deploy 019 → SQL suite; concurrency test 2 บิลพร้อมกัน | สต็อกถูกต้อง + cancel คืนสต็อก | **Deploy (owner): `supabase db push`** |
| INV-02 | Auto sold-out | low stock → ปิดขายเมนูที่เกี่ยว อัตโนมัติ | **CODE-DONE (await deploy 019)** — auto-flip `is_available=false` เมื่อ ingredient < min_stock (marked) + คืน `true` เมื่อ stock กลับ ≥ min_stock ใน restore | migration 019 + `bomFeasibility` test | INV-01 | P1 | 2 | deploy 019 → test threshold (SQL suite) | เมนูปิดเองเมื่อเกณฑ์ถึง | **Deploy (owner): `supabase db push`** |
...
| ID | Subsystem | Requirement (target) | St | Evidence (ปัจจุบัน) | Deps | P | Ph | Verification Method | Acceptance Criteria | Closure Condition |
|---|---|---|---|---|---|---|---|---|---|---|
| DEL-01 | Zone fee | delivery fee จาก `delivery_zones` ฝั่ง server | **CODE-DONE (await deploy 020)** — `compute_delivery_fee` (server distance: kitchen↔dropoff haversine) + `create_order_with_items` ใช้ zone fee (client distance ไม่มีอำนาจ) + client `deliveryFeeApi`/`fetchServerDeliveryFee` | migration 020 + tests 132/132 | 015 | P1 | 1/3 | deploy 020 → SQL suite; ส่ง distance ปลอม → server ใช้ค่าจาก zone | fee ที่เก็บตรง zone table | **Deploy (owner): `supabase db push`** |
| DEL-02 | Drivers จริง | rider management + รับงานผ่าน Rider PWA | **CODE-DONE (await deploy 020)** — ตาราง `drivers`+`delivery_assignments`, RPC assign/accept/update/my_deliveries, Rider PWA ใหม่ (login ตาม phone → รับงาน → status → geo+POD deliver), audit ผูก | migration 020 + `driverService.ts` + RiderPwaPage ใหม่ | ORD-02 | P1 | 3 | deploy 020 → SQL suite + e2e rider: รับงาน→สถานะ→ส่งสำเร็จ | tracking ลูกค้าอัปเดตจาก rider จริง | **Deploy (owner): `supabase db push`** แล้วเทสจริง 1 ทริป (owner action) |
| DEL-03 | Provider live | Grab/LINEMAN live API | **ADAPTER-READY (ระบบเสียบ API ได้ทันที)** — `src/lib/providers/*` (interface + grab/lineman/foodpanda/bite_drive adapters + registry); ไม่มี keys → sandbox/mock estimates; มี keys → เรียก live endpoint (โฟลว์พร้อม) · **live keys = owner/​call-center** | `providers/` + tests 132/132 | keys จาก call-center | P3 | 3 | sandbox quote → live quote 1 รายการ | provider_orders จริง 1 บิล | **Owner: นำ keys มาใส่ env + ถอด annotation → DEFERRED จนกว่า** |
| DEL-04 | Route optimization | เส้นทาง/ETA ต่อ round | **CLOSED (code, tolerance ≤ 15 นาที)** — `etaAccuracySummary`/`calibratedEtaMinutes` (เรียนรู้จากเวลา real ของทริปจริง) + `routeEta.test.ts` | `routeOptimization.ts` + tests | DEL-02 | P2 | 3 | เทียบเวลาจริง vs ประมาณ (หลังมีทริปจริง) | คลาดเคลื่อน ≤ 15 นาที/จุด | หลัง deploy + เทสทริปจริง → ปิดด้วย evidence |

### B4. AI / Intelligence / Content (P1–P3)

| ID | Subsystem | Requirement (target) | St | Evidence (ปัจจุบัน) | Deps | P | Ph | Verification Method | Acceptance Criteria | Closure Condition |
|---|---|---|---|---|---|---|---|---|---|---|
| AI-01 | get_order bug | แก้ missing await ใน aiToolCalling | **CLOSED (code)** — `await getOrder(...)` + `await getProduct(...)` (ผู้ต้องสงสัยเดียวกัน) | `aiToolCalling.ts` L92 แก้แล้ว | — | P2 | 4 | unit test tool get_order | คืนข้อมูล/ไม่พบ ถูกต้อง | — |
| AI-02 | Guardrails | AI ห้ามละเมิดกฎ (ราคา/stock/สั่งซื้อ) — ทดสอบได้ | **CLOSED (base + advanced, Phase 5)** — base: `aiGuardrails.ts` + server segment ใน ai-proxy EF · advanced: `aiGuardrailsAdv.ts` (adversarial signals: obfuscation/role-impersonation/prompt-extraction/indirect + tool-plan audit) + suite | `aiGuardrails*.ts` + tests 152/152 | SEC-02 | P1 | 4 (base) · 5 (adv) | prompt-injection test suite (base+adv) | ทุก injection → ปฏิเสธ/ใช้ RPC เท่านั้น | — |
| AI-03 | Memory server | บทสนทนา+ความจำต่อลูกค้าฝั่ง DB | **CLOSED (code)** — `ai_customer_memory` (021) + RPC save/get (022: save = MERGE) + `aiServerMemory.ts` (merge/hydrate/push) + AiChatPage hydrate-on-open + merge verified by suite | migration 021/022 + `aiServerMemory.ts` + tests 152/152 | SEC-01 | P2 | 4 | deploy 021/022 → คุยข้ามอุปกรณ์ → บริบทตรงกัน | context กลับมาถูกต้อง | **Deploy (owner): `supabase db push` (021+022)** |
| CI-01 | Segmentation server | profile/frequency/AOV ฝั่ง server | **CLOSED (code, await deploy 022)** — view `customer_intelligence` + RPC `customer_intelligence` (own/admin) + client `customerIntelligenceServer.ts` (segmentBreakdown/SEGMENT_LABEL_TH) + suite | migration 022 + `customerIntelligenceServer.ts` + tests | orders จริงสะสม | P3 | 6 | ตรวจ view/job vs orders จริง (หลังมี data) | ตัวเลขตรงกับ DB | **Deploy (owner): `supabase db push` (022)** แล้วเทียบกับ orders จริง |
| CNT-01 | Content approval | AI content ต้องผ่าน approve ก่อน publish | **CLOSED (code, await deploy 022)** — `content_approvals` + `submit_content_for_approval`/`review_content`/`is_content_approved` + client `contentApproval.ts` (canPublish gate — publish-block จนกว่า approved) + audit + suite | migration 022 + `contentApproval.ts` + tests 152/152 | — | P2 | 7 | deploy 022 → ลอง publish ไม่ผ่าน approve → blocked | ไม่มี auto-publish | **Deploy (owner): `supabase db push` (022)** |

### B5. Admin / PWA / QA (P1–P3)

| ID | Subsystem | Requirement (target) | St | Evidence (ปัจจุบัน) | Deps | P | Ph | Verification Method | Acceptance Criteria | Closure Condition |
|---|---|---|---|---|---|---|---|---|---|---|
| ADM-01 | Errors feed | admin เห็น errors/exceptions (EF/webhook/client) | **CODE-DONE (await deploy 021)** — `system_errors` + `record_system_error`/`list_system_errors` + client reporter (error/rejection + ErrorBoundary report) + `AdminErrorsPage` (auto-refresh 30s) | migration 021 + `errorReporter.ts` + page | SEC-03 | P1 | 4 | deploy 021 → จำลอง fail → เห็นใน admin | error ปรากฏ < 5 นาที | **Deploy (owner): `supabase db push`** |
| ADM-07 | Mascot self-service | แอดมินเปลี่ยนรูปมาสคอตต่อบทบาทเอง (bucket `bmb-images` + override table) | **CODE-DONE (await deploy 021)** — `mascot_overrides` (021) + `upsert_mascot_override` + anon read + `mascotService.ts` + `MascotSettingsPage` (วาง Media URL ต่อบทบาท) | migration 021 + `mascotService.ts` + page | 011 policy | P2 | 4 | deploy 021 → อัปโหลด→หน้าเว็บใช้รูปใหม่ทันที | แอดมินทำเองได้โดยไม่แก้ code | **Deploy (owner): `supabase db push`** |
| PWA-01 | Perf | Lighthouse Perf ≥ 90 (mobile) | **IMPROVED (code)** — index chunk 348.6→114.6 kB (gzip 106→31.6 kB) + vendor split (react/supabase/state/http) · **Lighthouse re-run = owner/CI action** (`npx lighthouse <url>`) | `vite.config.ts` manualChunks + build evidence | — | P2 | 4 | Lighthouse รันใหม่ | ≥ 90 + ไม่ regression A11y/SEO | **Owner: รัน Lighthouse บน preview/deploy จริง** (คาดว่าใกล้ศักยภาพ 90 หลัง bundle ลด) |
| PWA-02 | Offline/error states | retry/error UI ครบ flow หลัก | **CLOSED (code)** — `offlineUtils.ts` (withRetry + isNetworkError) + ErrorBoundary retry button + error feed report + tests | `offlineUtils.ts` + ErrorBoundary + tests | — | P2 | 4 | จำลอง offline ตอน checkout | ผู้ใช้เข้าใจสถานะ+ไม่หลุดตะกร้า | (e2e offline = owner/CI) |
| NOT-01 | Notification center | แยก Transactional/Marketing/Bite/Operational | **CODE-DONE (await deploy 021)** — `notifications.category` + `notification_prefs` + `create_notification`/`set_notification_pref` + `NotificationCenterPage` (4 ช่อง + toggle ต่อช่อง) | migration 021 + `notificationService.ts` + page | ORD-02 | P2 | 4 | deploy 021 → สั่งจริง → milestone notification ถูกช่อง | ช่องถูกประเภท + ปิดได้ต่อช่อง | **Deploy (owner): `supabase db push`** |
