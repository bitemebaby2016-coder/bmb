...
| SEC-03 | Audit (server) | audit log ฝั่ง DB ผูก auth.uid + RLS | MISSING (client-only) | `auditLog.ts` localStorage | migration ใหม่ | P1 | 1 | ตรวจตาราง + เขียนจาก RPC สำคัญ | ทุก order/payment transition มี row ฝั่ง DB | S-4 ปิด |
| SEC-04 | Key hygiene | retire legacy `SUPABASE_SERVICE_ROLE_KEY` env ใน EF | PARTIAL | stripe-webhook/create-checkout fallback | — | P2 | 0 | deploy EF ใหม่; ยิง smoke ผ่าน | code อ่าน env เดียว; smoke ยังผ่าน | S-5 ปิด |
| PAY-01 | Pre-order price | pre-order ใช้ server-side pricing (เทียบเท่า 007) | MISSING | `preOrderService.ts` ส่ง total_amount จาก client | ORD-01 pattern | P0 | 1 | test: ส่งราคาปลอม → server ละทิ้ง + ตรวจ DB | ทุก pre-order มีราคาจาก DB เท่านั้น | S-2 ปิด |
| PAY-02 | Card loop | บิลบัตรจริงครบวงจร 1 รายการบน production | PARTIAL | EF โค้ดครบ + webhook VERIFIED | Stripe live key | P0 | 1 | สั่งจริง 1 บิล → webhook → paid → DB verify | payment_intents completed + orders paid + receipt | หลักฐานบิลจริงแนบ |
| PAY-03 | Refund | refund จริง 1 รายการผ่าน EF admin | PARTIAL | EF + tests ผ่าน | PAY-02 | P0 | 1 | refund บิลจริง → ตรวจ Stripe + DB | intent refunded + บันทึก idempotent | หลักฐาน refund แนบ |
| PAY-04 | Order vocab | mapping สถานะชุดเดียว (server↔client↔provider) | PARTIAL | 3 vocabulary ใน code | ORD-01 | P1 | 1 | table-driven tests ของ mapping ทุกสถานะ | ไม่มีสถานะ orphan; UI แสดงตรง DB | vocabulary เดียวใน types |
...
| PRE-01 | Pre-order rounds | rounds จาก DB เท่านั้น (ลบ hardcoded client) | PARTIAL | `getPreOrderRounds()` hardcode | bmbAdminApi_rounds | P1 | 1 | e2e pre-order ใช้ round จาก DB | ไม่มี hardcode ใน client | — |
...
| QA-01 | CI | GitHub Actions: test+build ทุก push | MISSING | ไม่มี .github | — | P1 | 0/1 | push → workflow ผ่าน | 106 tests + build ใน CI | — |
| QA-02 | Lint | ติดตั้ง ESLint + script + ผ่าน 0 error | MISSING | npm run lint = Missing script | QA-01 | P2 | 0/1 | npm run lint ผ่าน | 0 error baseline | — |
| QA-03 | SQL contract tests | ทดสอบ RPC/trigger กับ Postgres จริง (กัน mock drift) | MISSING | mock จำลอง RPC | Supabase test project | P1 | 1 | รัน SQL tests กับ DB ที่ migrate 001→016 | ผลตรง mock ทุกข้อ | drift risk ปิด |
| QA-04 | e2e reproducible | ย้าย playwright เข้า repo devDeps + CI job | PARTIAL | ยืม node_modules โปรเจกต์อื่น | QA-01 | P3 | 1 | clone ใหม่ + npm i + e2e ผ่าน | ไม่พึ่ง path ภายนอก | — |
