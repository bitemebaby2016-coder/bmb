...
| AI-02 | Guardrails | AI ห้ามละเมิดกฎ (ราคา/stock/สั่งซื้อ) — ทดสอบได้ | **BASE CLOSED (code)** — `aiGuardrails.ts` (validateUserPrompt + assertReadOnlyTool) + server segment ใน ai-proxy EF + injection suite (`aiGuardrails.test.ts`) — **advanced suite = Phase 5** | `aiGuardrails.ts` + tests | SEC-02 | P1 | 4 (base) · 5 (adv) | prompt-injection test suite (base) | ทุก injection → ปฏิเสธ/ใช้ RPC เท่านั้น | — |
| AI-03 | Memory server | บทสนทนา+ความจำต่อลูกค้าฝั่ง DB | **CODE-DONE (await deploy 021)** — `ai_customer_memory` (021) + RPC save/get + `aiServerMemory.ts` (merge/hydrate/push) + AiChatPage hydrate-on-open — คุยข้ามอุปกรณ์เมื่อ deploy | migration 021 + `aiServerMemory.ts` + tests | SEC-01 | P2 | 4 | deploy 021 → คุยข้ามอุปกรณ์ → บริบทตรงกัน | context กลับมาถูกต้อง | **Deploy (owner): `supabase db push`** |
